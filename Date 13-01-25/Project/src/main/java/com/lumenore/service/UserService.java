...
public List<User> getAllUser() {
    ...
    for (User user : userList) {
        ...
        // Declare a ResponseEntity to hold the external API response
        ResponseEntity<ArrayList> responseArrayList = null;

        try {
            // Call the external service using RestTemplate
            responseArrayList = restTemplate.exchange(
            ...
            if (responseArrayList != null && responseArrayList.getBody() != null) {
                // If ratings data is found, map it to the user object
                ArrayList<Rating> rating = responseArrayList.getBody();
                if (rating != null && !rating.isEmpty()) {
                    user.setRatings(rating);
                } else {
                    user.setRatings(null);  // Set null if no ratings found
                }
            } else {
                user.setRatings(null);  // Set null if no ratings data is returned
            }
        } catch (Exception ex) {
            // Log the exception or print stack trace for debugging
            ex.printStackTrace();
            user.setRatings(null);  // Set ratings as null if there is an error
        }
    }

    // Return the list of users with updated ratings
    return userList;
}
